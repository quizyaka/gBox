...
function ControllerServers( $scope, $element, $rootScope, $location )
{
	...
	$scope.Type = {
		...
		"logo":{
		  "policerp":"police.png",
		  "terrortown":"ttt.png",
		  "mzstalker":"stalker.png",
		  "sandbox":"sandbox.png",
		  "starwarsrp":"starwars.png",
		  "darkrp":"darkrp.png",
		  "scprp":"scp.png",
		  "rust":"grust.png",
		  "basewars":"basewars.png",
		  "militaryrp":"military.png",
		  "urfimsup":"school.png",
		  "mangarp":"manga.png",
		  "zombiesurvival":"zombie.png"
		}
	  }
	...
	$scope.OpenServerInfo = function(server)
	{
		$scope.InfoMenu = true
	...
	{
		t = t.length
		// console.log(t, Math.round(200/t))
		t = Math.round(200/t)
		if (t > 16) t = 16
		else if (t < 14) {
			t = 14
	...
	$scope.GetType = function(gm)
	{
		gm = gm.toLowerCase()
		let r 
		// if gm.toLowerCase().indexOf('RP') != 0 or gm.indexOf('рп')
		if (gm.indexOf('rp') != -1 || gm.indexOf('рп') != -1) return 'Role Play'
		else if ($scope.Type.name[gm]) return $scope.Type.name[gm]
		// else if (gm.indexOf('basewars') != -1) return "PVP"
		else return 'Other'
	}

	$scope.GetTypeLogo = function(gm)
	{
		gm = gm.toLowerCase()
		if ($scope.Type.logo[gm]) {
			// console.log(gm)
			return $scope.Type.logo[gm]
		}
		else return "other.png"
	}
	$scope.ServerFont = function(s)
	{
		s = s.name.length
		s = 540/s*1.5
		if (s > 18) s = 18
	...
	$scope.Ping = function(ping)
	{
		// let tbl = ["#AEFF47","#AEFF47","white"]
		let f, s, t, f1, s1, t1
		// if ping
	...
	$scope.GamemodeName = function( gm )
	{
		...
		if ( gm.info && gm.info.title ) {
			let name = gm.info.title.replace(/([\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g,"").replace(/[^a-zа-яё0-9\s]/gi, '')
				if (name.length < 1) return "Unknown Gamemode"
			return name;
			// return gm.info.title
		}
		let goodname = gm.name
		goodname = goodname
		return goodname;
	}
