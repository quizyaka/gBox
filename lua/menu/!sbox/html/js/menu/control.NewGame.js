...
function ControllerNewGame( $scope, $element, $rootScope, $location, $filter )
{
	...
	$scope.GetActiveMap = function() 
	{	
		// console.log(localStorage)
		var map = $rootScope.Map
		var map_table = map.split('_')
		var out = ''
		if (map_table.length > 2)
		{
			for (var i = 2; i <= map_table.length; i++) out = out +map_table[i-1] + '_' 
			out = out.slice(0, -1)
			return out
		} else {
			out = map_table[1]
	...
	$scope.SelectGamemode = function( gm )
	{
		...
		lua.Run( "RunConsoleCommand( \"gamemode\", %s )", gm );
	...
	let range_help = document.getElementById('range_help')
	let range = document.getElementById('range')
	// range_help.addEventListener('change',function()
	// {
	...
	// 		range.value = this.value
	// 	}
	// 	range.style = 'background-size: '+range.value+ '% 100%;'
	// 	$scope.Val = range_help.value
	// })
	...
	$(document).on('input change', 'input[type="range"]', function() {
		...
		$rootScope.MaxPlayers  = this.value
		$('#range_help').val(this.value)
		$('#range').attr('style','background-size: '+this.value+'% 100%;')
	});
	...
	// {
	// 	this.style = 'background-size: '+range.value+ '% 100%;'
	// 	$scope.Val = this.value
	...
	$scope.MapIcon = function( m, cat )
	{
		...
		if ( !IN_ENGINE || $scope.CurrentCategory != cat ) return "img/sbox/downloading.png"
		let s = 'asset://mapimage/'
		let whitelist = [
			'gm_construct',
			'gm_flatgrass',
			'cs_assault',
		...
		if (whitelist.indexOf(m) == -1) return 'img/sbox/downloading.png' 
		return s + m;
		// return "img/sbox/downloading.png";
