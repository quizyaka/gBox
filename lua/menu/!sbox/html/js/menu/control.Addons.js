...
function ControllerAddons( $scope, $element, $rootScope, $location )
{
	...
	$scope.Cat = 0
	$scope.SwitchCat = function(arg)
	{
		$scope.Cat = arg
	}

	$scope.GoodTitle = function(arg)
	{
		if (arg == undefined) return arg
		if (arg.length >= 17) {
			return arg.substr(0, 17) + '...'
		}
		return arg
	}

	$scope.PrettyFont = function(str)
	{
		if (str == undefined) return

		if (str.search(/[а-яё]/i)>=0){
	...
	$scope.AddonImg = function(data)
	{
		if (data) {
			return 	'asset://garrysmod/'+data
		}
		return 'asset://garrysmod/lua/menu/!sbox/html/img/downloading.png'
	}
